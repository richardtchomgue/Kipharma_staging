...
class ResPartner(models.Model):
    ...
    def create(self, vals):
        ...
        if partner and not partner.vat and partner.country_code == 'RW':
            raise ValidationError("Please provide a Tax ID for customer.")
        return partner

    @api.onchange('country_id')
    def _vs_onchange_country_id(self):
        if not self.vat and self.country_code == 'RW':
            raise ValidationError("Please provide a Tax ID for customer.")

    def write(self, vals):
        for partner in self:
            if 'vat' in vals and partner.vat != vals['vat']:
                res = super(ResPartner, partner).write(vals)
                partner.get_customer_details()
            else:
                res = super(ResPartner, partner).write(vals)
            return res
