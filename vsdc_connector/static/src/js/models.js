odoo.define("vsdc_connector.models", function(require) {
    ...
    const VSDCOrderLine = (Orderline) => class VSDCOrderLine extends Orderline {
        get_tax_base(){
        ...
        get_all_prices(qty = this.get_quantity()){
            ...
            var all_taxes = this.pos.compute_all(product_taxes, price_unit, qty, this.pos.currency.rounding);
            var all_taxes_before_discount = this.pos.compute_all(product_taxes, this.get_unit_price(), qty, this.pos.currency.rounding);
        ...
        export_for_printing() {
            ...
            json.product_item_code = this.product.item_code;
            return json;
        }

    }
    Registries.Model.extend(Orderline, VSDCOrderLine);

    const VSDCPosGlobalState = (PosGlobalState) => class VSDCPosGlobalState extends PosGlobalState {
    ...
    Registries.Model.extend(PosGlobalState, VSDCPosGlobalState);

    const VSDCOrder = (Order) => class VSDCOrder extends Order {
        init_from_JSON(json) {
        ...
        async refresh_stamp(refund=false, refund_reason='') {
            var self = this;
            await ajax.jsonRpc(
                '/get-receipt-stamp',
                ...
                let code = data["code"]
                if (data["stamp"]){
                    if (data["stamp"]["synced_date"]){
                        data["stamp"]["synced_date"] = field_utils.format.datetime(moment(new Date(data["stamp"]["synced_date"])), {}, {timezone: true});
                    }
                }
    ...
    Registries.Model.extend(Order, VSDCOrder);
