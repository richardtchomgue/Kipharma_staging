...
class VSDCProductImport(models.Model):
    ...
    @api.model
    def action_fetch_vsdc_products(self, last_date=None):
        cookies = request.httprequest.cookies
        company_id = self.env['res.company'].sudo()
        if last_date:
            last_req_date = last_date.strftime('%Y%m%d%H%M%S')
        else:
            last_req_date = company_id.last_request_date
        if cookies.get('cids'):
            cid = cookies.get('cids').split(',')[0]
            if cid:
        ...
        req_data = {"tin": company_id.vat, "bhfId": company_id.vsdc_branch_id.branch_id or self.env.company.branch,
                    "lastReqDt": last_req_date}
        res = Messenger(self.env.user, req_data, company=self.env.company,
                        endpoint='items/selectItems').recv_vsdc_items()
        ItermSudo = self.env['vsdc.product.import'].sudo()
        item_ids = []
        if isinstance(res, dict) and res.get('resultCd') == "000":
            ...
            for item in item_list:
                ...
                if not item_exists:
                    item['company_id'] = company_id.id
                    item['request'] = req_data
                    item['response'] = res
                    items.append(item)
            if items and isinstance(items, list):
                item_ids = ItermSudo.create(items)
        return item_ids
