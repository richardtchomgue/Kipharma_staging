...
class Miner:
    ...
    def get_sale_receipt_data(self, invoice, on_confirmation=None):
        ...
        receipt = {
            ...
            "trdeNm": invoice.company_id.name[:20] or None,
            "adrs": invoice.company_id.partner_id.contact_address or None,
            "topMsg": invoice.company_id.top_message or None,
            "btmMsg": invoice.company_id.bottom_message or None,
            "prchrAcptcYn": "Y"
        }
        data = {
            "tin": invoice.company_id.vat,
            "bhfId": invoice.company_id.branch_id,
            "invcNo": invoice.receipt_number,
            "orgInvcNo": f"{invoice.id}{invoice.name.split('/')[2] if len(invoice.name.split('/'))>2 else '/'}" if invoice.name else invoice.id,
            "custTin": invoice.partner_id.vat,
    ...
    def get_sale_receiptitem_data(self, line, index, on_confirmation=None):
        ...
        data = {
            ...
            "qty": line.quantity,
            "prc": line.price_unit,
            "splyAmt": line.price_unit * line.quantity,
    ...
    def get_item_data(self, product, company):
        ...
        data = {
            ...
            'taxTyCd': product.taxes_id[0].rra_code if product.taxes_id else 'B',
            'btchNo': None,
    ...
    def get_inventory_data(self, picking):
        ...
        for index, line in enumerate(picking.move_ids_without_package):
            ...
            line_data = {
                ...
                "itemExprDt": None,
                "prc": prc,
                "splyAmt": _special(prc * line.quantity_done),
                "totDcAmt": discount,
    ...
    def get_purchase_data(self, invoice):
        ...
        data = {
            ...
            "totItemCnt": len(invoice.invoice_line_ids.filtered(lambda l: l.display_type=='product')),
            "taxblAmtA": invoice.taxable_a,
            "taxblAmtB": invoice.taxable_b,
            "taxblAmtC": invoice.taxable_c,
            "taxblAmtD": invoice.taxable_d,
    ...
    def get_purchaseitem_data(self, line, index, ):
        ...
        data = {
           ...
           "splyAmt": _special(line.product_id.standard_price * line.quantity),
    ...
    def get_tax_details(self, lines, company, vendor, is_refund=False):
        ...
        for line in lines:
            for tax in line.tax_ids:
                vals = tax.compute_all(price_unit=line.price_unit, currency=company.currency_id, partner=vendor,
