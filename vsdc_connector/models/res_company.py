...
class ResCompany(models.Model):
    ...
    last_request_datetime = fields.Datetime(string='Last Request Date')
    last_request_date = fields.Char(compute="_compute_date_str")
    top_message = fields.Char(string='Top Message')
    bottom_message = fields.Char(string='Bottom Message')
    ...
    # VSDC Courier
    last_sale_invoice_number = fields.Char(string="Last Sale Invoice No", required=False)
    ...
    @api.depends('last_request_datetime')
    def _compute_date_str(self):
        last_request_datetime = fields.Datetime.now()
        if self.last_request_datetime:
            last_request_datetime = self.last_request_datetime
        last_request_date = last_request_datetime.strftime('%Y%m%d%H%M%S')
        self.last_request_date = last_request_date

    def show_message(self, message, name="Device Status"):
        return {
    ...
    def action_update_company_details(self):
        ...
        self.name = self.company_name if self.company_name else self.branch_name
    ...
    def action_import_vsdc_branches(self):
        ...
        self.import_branches_request = data
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if json_response.get('resultCd', '') == '000':
                    ...
                    return self.show_message(message=f"[{branches_count}] VSDC Branches Created Successfully.", name="VSDC Branches Response")
                else:
    ...
    def action_import_essentials(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self.show_message(message=json_response.get('resultMsg', ''), name="Essentials Response")
        except Exception as e:
            raise ValidationError(f'VSDC Exception for ({url}): {e}')
    ...
    def action_import_item_class(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self.show_message(message=json_response.get('resultMsg', ''), name="Item Classifications Response")
        except Exception as e:
            raise ValidationError(f'VSDC Exception for ({url}): {e}')
...
class ResUser(models.Model):
    ...
    mrc = fields.Char(string="MRC", help='Machine Registration Code')
