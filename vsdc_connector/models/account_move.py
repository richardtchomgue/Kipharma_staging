...
class AccountMove(models.Model):
    ...
    receipt_number = fields.Char(readonly=True, default=0, string="Receipt ID", copy=False)
    ...
    stamp = fields.Many2one('account.move.stamp', compute='compute_stamp', store=True)
    ...
    taxable_a = fields.Float(compute='compute_tax_amounts', store=True)
    taxable_b = fields.Float(compute='compute_tax_amounts', store=True)
    taxable_c = fields.Float(compute='compute_tax_amounts', store=True)
    taxable_d = fields.Float(compute='compute_tax_amounts', store=True)
    ...
    amount_tax_a = fields.Float(compute='compute_tax_amounts', store=True)
    amount_tax_b = fields.Float(compute='compute_tax_amounts', store=True)
    amount_tax_c = fields.Float(compute='compute_tax_amounts', store=True)
    amount_tax_d = fields.Float(compute='compute_tax_amounts', store=True)
    ...
    def create(self, vals):
        ...
        if res and res.company_id:
            last_invoice_number = res.company_id.last_sale_invoice_number
            if last_invoice_number:
                res.receipt_number = str(int(last_invoice_number)+1)
    ...
    def _send_invoice_to_vsdc(self):
        ...
        try:
            ...
            if isinstance(res, dict) and res.get('resultCd') == "000":
                ...
                self.company_id.last_sale_invoice_number = self.receipt_number
                ...
                data = res.get('data', {})
                res_date = res.get('resultDt', '') or data.get('vsdcRcptPbctDate', '')
...
class AccountMoveLine(models.Model):
    ...
    price_reduce = fields.Float(compute='compute_price_reduce')
    recv_line_id = fields.Many2one('account.move.line.recv', 'Supplier Invoice Line', ondelete='restrict')
    import_item_id = fields.Many2one('recv.import.item', 'Import Item', ondelete='restrict')
    amount_tax = fields.Float(compute='compute_tax_vals', store=True)
